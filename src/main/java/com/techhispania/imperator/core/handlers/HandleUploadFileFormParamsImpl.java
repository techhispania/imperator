 ...
 */
public class HandleUploadFileFormParamsImpl implements HandleFormParams {

	private static final Logger logger = LogManager.getLogger(HandleUploadFileFormParamsImpl.class);
		...
		while (pos < bodyBytes.length) {
			...
			if (headerEnd > 0) {
				...
				if (headers.contains("filename=")) {
					...
					executeControllerMethod(bodyBytes, filename, exchange, method, controller, requestBodyType);
				}
			}
			pos = nextBoundary;
		}
	...
	private void executeControllerMethod(byte[] bodyBytes, String filename, HttpExchange exchange, Method method, Object controller, Class<?> requestBodyType) throws Exception {
		String body = new String(bodyBytes);
		Map<String, String> formParams = parseMultipartFormFields(body, retrieveBoundary(exchange).get());
	...
	private Map<String, String> parseMultipartFormFields(String body, String boundary) {
	    ...
	    for (String part : parts) {
	        ...
	        fields.put(fieldName, URLDecoder.decode(content, StandardCharsets.UTF_8));
	    }
	    return fields;
	}
	
	private Optional<String> retrieveBoundary(HttpExchange exchange) {
